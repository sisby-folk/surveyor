...
public class SurveyorClientNetworking {
    ...
    private static void handleExploredStructures(ClientWorld world, WorldSummary summary, SyncExploredStructuresPacket packet) {
        packet.structureKeys().forEach((key, starts) -> SurveyorClient.ClientExploration.INSTANCE.surveyor$exploredStructures().computeIfAbsent(packet.worldKey(), k -> new HashMap<>()).computeIfAbsent(key, k -> new LongOpenHashSet()).addAll(starts));
    }
